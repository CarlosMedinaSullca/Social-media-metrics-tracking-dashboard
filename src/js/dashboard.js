...
import {
  ...
  processAdSetsData,
  processAdsData,
} from "./dataProcessing.mjs";
import { addStyles } from "./styles.mjs";
import {
  createAdSetsTableHTML,
  createAdsTableHTML,
  createCampaignTableHTML,
  campaignSummary,
} from "./tableHelpers.mjs";
...
async function initDashboard() {
  ...
  try {
    ...
    if (!response.data || response.data.length === 0) {
      ...
      throw new Error(
        "No campaign data available. Please check your ad account or use mock data.",
      );
    }
